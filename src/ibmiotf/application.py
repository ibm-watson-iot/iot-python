...
from ibmiotf.codecs import utf8Codec
from ibmiotf.codecs import binaryCodec
import ibmiotf.api
...
class Client(ibmiotf.AbstractClient):
	...
	It accepts 5 parameters, deviceType, deviceId, event which denotes event type,
	data which is the message to be posted and the data format. dataFormat can
	be one of these - json, text, xml and bin. The defaut is json.
	...
	'''
	def publishEventOverHTTP(self, deviceType, deviceId, event, data,dataFormat="json"):
		self.logger.debug("Sending event %s with data format %s" % (event, dataFormat))

		templateUrl = 'https://%s.%s/api/v0002/application/types/%s/devices/%s/events/%s'
...
#		String replacement from template to actual URL
		...
		try:
			...
			self.logger.debug("Open SSL Version:")
			self.logger.debug(ssl.OPENSSL_VERSION)
			...
			self.logger.debug(payload)
			self.logger.debug("Auth-key:")
			self.logger.debug(authKey)
			self.logger.debug("Auth-Token:")
			self.logger.debug(authToken)
	...
	It accepts 5 parameters, deviceType, deviceId, event which denotes event type,
	command which is the command to be posted and the command format. cmdFormat can
	be one of these - json, text, xml and bin. The defaut is json.
	It throws a ConnectionException with the message "Server not found" if the
	application is unable to reach the server, Otherwise it returns the
	HTTP status code, (200 - 207 for success)
	'''
	def publishCommandOverHTTP(self, deviceType, deviceId, event, cmdData,cmdFormat="json"):
		self.logger.debug("Sending event %s with command format %s" % (event, cmdFormat))

		templateUrl = 'https://%s.%s/api/v0002/application/types/%s/devices/%s/commands/%s'
...
#		String replacement from template to actual URL
		...
		try:
			contentType = getContentType(cmdFormat)
			self.logger.debug("Open SSL Version:")
			self.logger.debug(ssl.OPENSSL_VERSION)
			self.logger.debug("Content-type:")
			self.logger.debug(contentType)
			...
			self.logger.debug(intermediateUrl)
			response = requests.post(intermediateUrl, auth = credentials, data = payload, headers = {'content-type': contentType})
			self.logger.debug("Status Code returned for publishCommandOverHTTP: ")
			self.logger.debug(response.status_code)
		except Exception as e:
			self.logger.error("POST Failed")
			self.logger.error(e)
