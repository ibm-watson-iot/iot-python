...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-t", "--typeId", required=False, default="iotpsutil", help="Set the typeId for the device batch.  Defaults to iotpsutil"
    )
    parser.add_argument(
        "-b", "--batchId", required=True, help="DeviceIDs will be prefixed by the batch number, e.g. batchID-0001, batchID-0002"
    )
    parser.add_argument(
        "--numberOfDevices", required=True, type=int, help="How many devices in the batch should be deployed. Max value is 1000"
    )

    parser.add_argument(
        "--helmCmd", required=False, default="helm", help="If helm command is not on the path, use this to set the helm command explicitly"
    )
    parser.add_argument(
        "--helmChart", required=True, help="Set the helm chart to use"
    )
    ...
    scriptFilePath = os.path.join(scriptDirectory, "deploy-%s-%s-%s.bat" % (args.classId.lower(), args.typeId, args.batchId))
    with open(scriptFilePath, "w") as scriptFile:
        for command in commands:
            scriptFile.write(command + "\n")

    scriptFilePath2 = os.path.join(scriptDirectory, "recall-%s-%s-%s.bat" % (args.classId.lower(), args.typeId, args.batchId))
