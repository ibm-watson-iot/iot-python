...
def manageLi(liName, liAlias, description, schemaId):
    ...
    if existingLi is None:
        ...
        createdLI = client.state.draft.logicalInterfaces.create({"name": liName, "alias": liAlias, "description": description, "schemaId": schemaId})
        print(createdLI)
        print("")
        li = createdLI
    else:
        print("LI %s already exists" % (liName))
        existingLi = client.state.draft.logicalInterfaces.update(existingLi.id, {"id": existingLi.id, "name": liName, "alias": liAlias, "description": description, "schemaId": schemaId})
        print(existingLi)
        print("")
        li = existingLi
...
def addMappings(deviceType, mappings):
    for mapping in mappings:
        if mapping["logicalInterfaceId"] in deviceType.mappings:
            ...
            print("Mapping for LI %s needs to be created in device type %s" % (mapping["logicalInterfaceId"], deviceType.id))
...
if __name__ == "__main__":
    ...
    # =========================================================================
    sysutilSchema = manageLiSchema(schemaName="sysutil-lischema", schemaFileName="sysutil-schema.json", description="Schema that defines the canonical interface for anything capable of reading System Utilization data")
    networkioSchema = manageLiSchema(schemaName="networkio-lischema", schemaFileName="networkio-schema.json", description="Schema that defines the canonical interface for anything capable of reading Network I/O data")

    # =========================================================================
    # Create the logical interface
    # =========================================================================
    sysutilLI = manageLi(liName="sysutil-li", liAlias="sysutil", description="System Utilization Logical Interface", schemaId=sysutilSchema.id)
    networkioLI = manageLi(liName="networkio-li", liAlias="networkio", description="Network I/O Logical Interface", schemaId=networkioSchema.id)
