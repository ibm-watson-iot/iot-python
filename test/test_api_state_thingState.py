...
class TestThing(testUtils.AbstractTest):
    ...
    def createAndCheckThing(self, thingTypeId, thingId, name, description, aggregatedObjects, metadata =None ):
        createdThing =TestStateUtils.createThing(self.appClient, thingTypeId, thingId, name, description, aggregatedObjects, metadata)
    ...
    def testCreatePreReqs(self):
        ...
        TestThing.createdEventType = self.createEventType(
            ...
            TestThing.createdEventSchema.id)

        # Create a Physical Interface
        TestThing.createdPI = self.createPI(
            testPIName, 
            "Test Physical Interface description")      
        
        # Associate event with PI
        TestThing.createdPI.events.create({"eventId": TestThing.testEventId, "eventTypeId": TestThing.createdEventType.id})
           
        test_dt_name = TestThing.testDeviceTypeName
        assert self.doesDTNameExist(test_dt_name)==False
        
        # Create a Device Type
        TestThing.createdDT = self.createAndCheckDT(
            test_dt_name, 
        ...
        testThingTypeSchema ={
            ...
            "properties" : {
                "Temperature" : {
                    "type" : "object",
                    "description" : "info",        
                    "$logicalInterfaceRef" : TestThing.createdLI.id
                    }
                },      
            "required" : ["Temperature"]
            }
        ...
        LItest = TestThing.createdTT.logicalInterfaces.create({
            "id": TestThing.createdThingLI.id})
        ...
        #validate thingType        
        print(self.appClient.state.draft.thingTypes[TestThing.createdTT.id].validate())
        ...
        aggregated = {
            "Temperature": {
                "type": "device",
                "typeId": TestThing.createdDT.id,
                "id": TestThing.createdDevice.deviceId
                }
            }
        
        #Create the thing
        TestThing.createdThing = self.createAndCheckThing(
            TestThing.createdTT.id,
            thingId,
            thingName,
            thingDescription,
            aggregated,
        ...
        assert TestStateUtils.doesThingIdExist(self.appClient,TestThing.thingTypeId,  TestThing.createdThing.thingId)  
        
                
                
    def testDefaultThingState(self):
        found = False
        for li in TestThing.createdTT.logicalInterfaces:
            # print("LI: %s" % (li.id))
            if li.name == TestThing.testThingLISchema:
                thingState = TestThing.createdThing.states[li.id]
                self.checkState (thingState, TestThing.defaultThingState)
                found = True
        print("Found an LI name: %s?: %s" % (TestThing.testThingLISchema, found)) 
        assert found


    def testResetThingState(self):
        found = False
        for li in TestThing.createdTT.logicalInterfaces:
            # print("LI: %s" % (li.id))
            if li.name == TestThing.testThingLISchema:
                TestThing.createdThing.states[li.id].reset()
                thingState = TestThing.createdThing.states[li.id]
                self.checkState (thingState, TestThing.defaultThingState)
                found = True
        print("Found an LI name: %s?: %s" % (TestThing.testThingLISchema, found))    
    ...
    def testThingStateErrors(self):
        ...
        try:
            for thingState in TestThing.createdThing.states:
                print("We shouldn't be able to iterate over thing state for LIs. We have: %s" % thingState)
                assert False==True # fail
        except:
             assert True # This is what we expect
        ...
        # Try to 'find' state
        try:
            for thingState in TestThing.createdThing.states.find({"name": ""}):
                print("We shouldn't be able to iterate over thing state for LIs. We have: %s" % thingState)
                assert False==True # fail
        except:
             assert True # This is what we expect
