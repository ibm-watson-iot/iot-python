...
##########################################################################
    ...
    @raises(Exception)
    def testGetSchemaErr(self):
        with assert_raises(APIException) as e:
            apiClient = ibmiotf.api.ApiClient({"auth-method": "token","auth-token": self.authToken,
                                               "auth-key": self.authKey},self.logger)
            apiClient.getSchema()
        assert_equal(e.exception.msg, 'HTTP error in getting a schema')

    @raises(Exception)
    def testGetSchemasErr(self):
        with assert_raises(APIException) as e:
            apiClient = ibmiotf.api.ApiClient({"auth-method": "token","auth-token": self.authToken,
                                               "auth-key": self.authKey},self.logger)
            apiClient.getSchemas()
        assert_equal(e.exception.msg, 'HTTP error getting all schemas')

    @raises(Exception)
    def testGetSchemaContentErr(self):
        with assert_raises(APIException) as e:
            logger.info("# ---- Fetch a schema content -------")
            apiClient = ibmiotf.api.ApiClient({"auth-method": "token","auth-token": self.authToken,
                                               "auth-key": self.authKey},self.logger)
            apiClient.getSchemaContent("event1 schema")
        assert_equal(e.exception.msg, 'No such schema Content retrieved')


    @raises(Exception)
    def testCreateSchemaErr(self):
        with assert_raises(APIException) as e:
            ids = {}
            logger.info("# ---- add an event schema -------")
            infile = open("event2.json")
            schemaFileContents = ''.join([x.strip() for x in infile.readlines()])
            infile.close()
            apiClient = ibmiotf.api.ApiClient({"auth-method": "token","auth-token": self.authToken,
                                               "auth-key": self.authKey},self.logger)
            ids["event2 schema"], apiClient.createSchema("event2 schema", 'event2.json', schemaFileContents)
            logger.info("# ---- Failed to create a Schema -------")
        assert_equal(e.exception.msg, 'HTTP error creating a schema')
    ...
    def testCreateSchema(self):
        with assert_raises(APIException) as e:
            ...
            apiClient = ibmiotf.api.ApiClient({"auth-method": "token","auth-token": self.authToken,
                                               "auth-key": self.authKey},self.logger)
            ids["event1 schema"], result = apiClient.createSchema("event1 schema", 'event1.json', schemaFileContents)
            print(result)
        assert_equal(e.exception.msg, 'Schema successfully created')

    @raises(Exception)
    def testGetSchema(self):
        with assert_raises(APIException) as e:
            logger.info("# ---- Fetch a schema -------")
            apiClient = ibmiotf.api.ApiClient({"auth-method": "token","auth-token": self.authToken,
                                               "auth-key": self.authKey},self.logger)
            apiClient.getSchema("event1 schema")
        assert_equal(e.exception.msg, 'One schema retrieved')

    @raises(Exception)
    def testGetSchemas(self):
        logger.info("# ---- Fetch all schemas -------")
        with assert_raises(APIException) as e:
            apiClient = ibmiotf.api.ApiClient({"auth-method": "token","auth-token": self.authToken,
                                               "auth-key": self.authKey},self.logger)
         #   apiClient.getSchemas()
        #assert_equal(e.exception.msg, 'All schemas retrieved')
        assert_true(apiClient.getSchemas())

    @raises(Exception)
    def testGetSchemaContent(self):
        with assert_raises(APIException) as e:
            logger.info("# ---- Fetch a schema content -------")
            apiClient = ibmiotf.api.ApiClient({"auth-method": "token","auth-token": self.authToken,
                                               "auth-key": self.authKey},self.logger)
            apiClient.getSchemaContent("event1 schema")
        assert_equal(e.exception.msg, 'One schema Content retrieved')
...
##########################################################################
    ...
    @raises(Exception)
    def testGetEventTypeErr(self):
        logger.info("# ---- Fetch all event types -------")
        with assert_raises(APIException) as e:
            apiClient = ibmiotf.api.ApiClient({"auth-method": "token","auth-token": self.authToken,
                                               "auth-key": self.authKey},self.logger)
            apiClient.getEventType("eventtype1")
        assert_equal(e.exception.msg, 'Event type eventtype1 not found')

    @raises(Exception)
    def testGetEventTypesErr(self):
        logger.info("# ---- Fetch all event types -------")
        with assert_raises(APIException) as e:
            apiClient = ibmiotf.api.ApiClient({"auth-method": "token","auth-token": self.authToken,
                                               "auth-key": self.authKey},self.logger)
            apiClient.getEventTypes()
        assert_equal(e.exception.msg, 'No event types retrieved')

    @raises(Exception)
    def testCreateEventType(self):
        logger.info("# ---- Create an Event type -------")
        with assert_raises(APIException) as e:
            apiClient = ibmiotf.api.ApiClient({"auth-method": "token","auth-token": self.authToken,
                                               "auth-key": self.authKey},self.logger)
            ids["k64feventtype"], result = apiClient.createEventType("K64F event", ids["event1 schema"], "K64F event")
            print(result)
        assert_equal(e.exception.msg, 'Event type successfully created')

    @raises(Exception)
    def testGetEventType(self):
        logger.info("# ---- Fetch specific event type -------")
        with assert_raises(APIException) as e:
            apiClient = ibmiotf.api.ApiClient({"auth-method": "token","auth-token": self.authToken,
                                               "auth-key": self.authKey},self.logger)
            apiClient.getEventType("k64feventtype")
        assert_equal(e.exception.msg, 'Event type successfully retrieved')
...
##########################################################################
    ...
    @raises(Exception)
    def testGetPhysicalInterfacesErr(self):
        logger.info("# ---- Fetch all event types -------")
        with assert_raises(APIException) as e:
            apiClient = ibmiotf.api.ApiClient({"auth-method": "token","auth-token": self.authToken,
                                               "auth-key": self.authKey},self.logger)
            output = apiClient.getPhysicalInterfaces()
            print(output)
            #if output == None:
		#assert_true(e.exception.msg, 'HTTP error getting all physical interfaces')
        assert_equal(e.exception.msg, 'HTTP error getting all physical interfaces')

    @raises(Exception)
    def testGetPhysicalInterfaceErr(self):
        logger.info("# ---- Fetch all event types -------")
        with assert_raises(APIException) as e:
            apiClient = ibmiotf.api.ApiClient({"auth-method": "token","auth-token": self.authToken,
                                               "auth-key": self.authKey},self.logger)
            assert_false(apiClient.getPhysicalInterface("K6F"))
            #apiClient.getPhysicalInterface("K6F")
        #assert_equal(e.exception.msg, 'HTTP error getting a physical interface')

    @raises(Exception)
    def testCreatePhysicalInterface(self):
        logger.info("# ---- Fetch all event types -------")
        with assert_raises(APIException) as e:
            apiClient = ibmiotf.api.ApiClient({"auth-method": "token","auth-token": self.authToken,
                                               "auth-key": self.authKey},self.logger)
    ...
    @raises(Exception)
    def testCreatePhysicalInterfaceErr(self):
        logger.info("# ---- Fetch all event types -------")
        with assert_raises(APIException) as e:
            apiClient = ibmiotf.api.ApiClient({"auth-method": "token","auth-token": self.authToken,
                                               "auth-key": self.authKey},self.logger)
            ids["physicalinterface"], result = api.createPhysicalInterface("K64F", "The physical interface for K64F example")
            if result:
                assert_false(e.exception.msg, 'HTTP error creating physical interface')
        #assert_equal(e.exception.msg, 'HTTP error getting a physical interface')


    @raises(Exception)
    def testGetPhysicalInterface(self):
        logger.info("# ---- Fetch all event types -------")
        with assert_raises(APIException) as e:
            apiClient = ibmiotf.api.ApiClient({"auth-method": "token","auth-token": self.authToken,
                                               "auth-key": self.authKey},self.logger)
            assert_false(apiClient.getPhysicalInterface("K64F"))
            #apiClient.getPhysicalInterface("K6F")
        #assert_equal(e.exception.msg, 'HTTP error getting a physical interface')
...
##########################################################################
    ...
    def testGetEvents(self):
        ...
        with assert_raises(APIException) as e:
            apiClient = ibmiotf.api.ApiClient({"auth-method": "token","auth-token": self.authToken,
                                               "auth-key": self.authKey},self.logger)
            assert_true(apiClient.getEvents("K64F"), 'All event mappings retrieved')
        #assert_equal(e.exception.msg, 'HTTP error getting a physical interface')
