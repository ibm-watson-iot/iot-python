...
class TestDevice(testUtils.AbstractTest):
    ...
    def testCreatePreReqs(self):
        ...
        TestDevice.createdEventType = self.createEventType(
            ...
            TestDevice.createdEventSchema.id)

        # Create a Physical Interface
        TestDevice.createdPI = self.createPI(
            testPIName, 
            "Test Physical Interface description")      
        
        # Associate event with PI
        TestDevice.createdPI.events.create({"eventId": TestDevice.testEventId, "eventTypeId": TestDevice.createdEventType.id})
           
        test_dt_name = TestDevice.testDeviceTypeName
        assert self.doesDTNameExist(test_dt_name)==False
        
        # Create a Device Type
        TestDevice.createdDT = self.createAndCheckDT(
            test_dt_name, 
