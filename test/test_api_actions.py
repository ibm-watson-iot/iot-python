...
class TestActions(testUtils.AbstractTest):
    ...
    def activateDeviceType(self):
        ...
        TestActions.createdLI = self.createLI(
            ...
            TestActions.createdLISchema.id)      
        ...
        TestActions.createdEventType = self.createEventType(
            ...
            TestActions.createdEventSchema.id)

        # Create a Physical Interface
        TestActions.createdPI = self.createPI(
            testPIName, 
            "Test Physical Interface description")      
        
        # Associate event with PI
        TestActions.createdPI.events.create({"eventId": TestActions.testEventId, "eventTypeId": TestActions.createdEventType.id})
           
        test_dt_name = TestActions.testDeviceTypeName
        assert self.doesDTNameExist(test_dt_name)==False
        
        # Create a Device Type
        self.createdDT = self.createDT(
            test_dt_name, 
       ...
       # Mappings
        ...
        for attempt in range(6):
            if (self.doesActiveSchemaNameExist(TestActions.testEventSchemaName) and
               self.doesActiveSchemaNameExist(TestActions.testLiSchemaName) and
               self.doesActiveEventTypeNameExist(TestActions.testEventTypeName) and
    ...
    def deactivateDeviceType(self):
        ...
        for attempt in range(6):
            if not (self.doesActiveSchemaNameExist(TestActions.testEventSchemaName) or
               self.doesActiveSchemaNameExist(TestActions.testLiSchemaName) or
