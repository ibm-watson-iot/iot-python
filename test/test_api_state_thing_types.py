...
class TestThingTypes(testUtils.AbstractTest):
    ...
    def testCreatePreReqs(self):
        ...
        TestThingTypes.createdLI = TestStateUtils.createLI(
            ...
            TestThingTypes.createdLISchema.id)      
        ...
        TestThingTypes.createdEventType = TestStateUtils.createEventType(
            ...
            TestThingTypes.createdEventSchema.id)


    def testThingTypeActivation(self):
        # Check all the resources, we may have to wait for this to complete
        for attempt in range(6):
            if (TestStateUtils.doesSchemaNameExist(self.appClient, TestThingTypes.testEventSchemaName) and
               TestStateUtils.doesSchemaNameExist(self.appClient, TestThingTypes.testLiSchemaName) and
               TestStateUtils.doesEventTypeNameExist(self.appClient, TestThingTypes.testEventTypeName) and
            ...
            print ("Active? Event Schema: %s, LI Schema: %s, Event Type: %s, Logical Interface: %s" %
        ...
        TestThingTypes.createdTT = self.createAndCheckTT(
            TestThingTypes.thingTypeId,
            "temperature type", 
            "Test Device Type description",
            thingSchema.id)
        #create logical interface for thingType
        LItest = TestThingTypes.createdTT.logicalInterfaces.create({
            "id": TestThingTypes.createdLI.id})
